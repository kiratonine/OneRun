...
- Railway deploys the `base` branch with `backend/` as the service root.
...
- `LLM_PROVIDER`, `GEMINI_API_KEY`, `GEMINI_MODEL` — consumed by the report
  module. The default model is `gemini-3.1-flash-lite`.
